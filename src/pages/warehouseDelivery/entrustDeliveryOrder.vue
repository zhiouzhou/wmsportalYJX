    ...
    <div class="box" v-loading="loading">
      ...
      <div v-for="item in orderList" style="overflow-y:auto;height:520px;margin-top:10px;">
        <div class="table-header">
          <span style="padding-left: 5px">委托单编号：{{item.stockNo}}</span>
          <span style="padding-left:18px">创建时间：{{item.createTime}}</span>
          <span class="genre" style=" color: #0096FF;"
                v-if="item.businessType">{{item.businessType?'贷款业务':'正常业务'}}</span>
        ...
        <div class="table-content">
          ...
          <div class="table-content-state">
            ...
            <span v-if="item.state==1">新增</span>
            <span v-if="item.state==2">待配送</span>
            <span v-if="item.state==3">配送中</span>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        state: `0`,//委托配送状态： 待支付=0 新增=1 待配送=2 配送中=3 已完成=4 已取消=5
        stockNo: null,//订单编号
        mobileNo: '',//经销商手机
        loading:false,
    ...
    methods: {
      ...
      selectStockApplyNote() {
        ...
        let applyType = 2;//1：出库申请单，2：委托配送单
      ...
      gotoDetail(stockNo, dealerId) {
        this.$router.push({name: `entrustDeliveryDetail`, params: {stockNo, dealerId}})
      },
